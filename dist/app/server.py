...
try:
    ...
    from smart_content_detector import (
        find_content_start_page,
        detect_headers_footers,
        apply_header_footer_filter,
        filter_text_for_tts,
...
class PatchedKokoro(Kokoro):
    ...
    def _create_audio(self, phonemes: str, voice: np.ndarray, speed: float):
        ...
        # PROACTIVE FIX: Handle empty tokens to prevent downstream errors
        if len(tokens) == 0:
    ...
    def phonemize(self, text: str, lang: str):
        """
        Delegate phonemization to tokenizer.
        """
        return self.tokenizer.phonemize(text, lang)

    def create(self, text: str, voice: str, speed: float = 1.0, lang: str = "en-us"):
        """
        Robust create method that handles empty phonemization results
        and avoids 'need at least one array to concatenate' errors.
        """
        try:
            # 1. Phonemize (using our exposed method)
            phonemes = self.phonemize(text, lang)

            # 2. Check for empty phonemes
            if not phonemes or not phonemes.strip():
                return np.zeros(int(SAMPLE_RATE * 0.1), dtype=np.float32), SAMPLE_RATE

            # 3. Direct synthesis if within ID limit (Standard Kokoro limit is ~510 tokens)
            # We use a safe margin. If it fits, we synthesize directly to avoid
            # the buggy splitting logic in super().create()
            ...
            # 4. Fallback to base class for long text
            return super().create(text, voice, speed, lang)

        except ValueError as e:
            if "need at least one array to concatenate" in str(e):
                # Attempt fallback: Text might have problematic punctuation
                try:
                    # Remove all non-alphanumeric chars (keep spaces)
                    clean_text = "".join(c for c in text if c.isalnum() or c.isspace())
                    if clean_text.strip() and clean_text != text:
                        print(
                            f"[PatchedKokoro] Retrying with cleaned text: '{clean_text[:20]}...'"
                        )
                        return super().create(clean_text, voice, speed, lang)
                except Exception:
                    pass

                # Return 0.1s silence for empty/unrecoverable segments
                return np.zeros(int(SAMPLE_RATE * 0.1), dtype=np.float32), SAMPLE_RATE
            raise e
        except AttributeError:
            # Fallback if internal attributes change
            return np.zeros(int(SAMPLE_RATE * 0.1), dtype=np.float32), SAMPLE_RATE
        except Exception as e:
            # Fallback for other errors in synthesis
            print(f"[PatchedKokoro] Synthesis failed for '{text[:20]}...': {e}")
            return np.zeros(int(SAMPLE_RATE * 0.1), dtype=np.float32), SAMPLE_RATE
...
async def serve_ui():
    ...
    if not ui_file.exists():
        return JSONResponse(
            status_code=500,
            content={
                "error": "UI file missing. Please check installation.",
                "path": str(ui_file),
            },
        )
...
async def get_locale(lang: str):
    ...
    if not locale_path.exists():
        raise HTTPException(
            status_code=404, detail=f"Translation file not found: {lang}.json"
        )
...
async def get_available_voices():
    ...
    if not voices_path.exists():
        return {"model": "none", "voices": {}, "size_mb": 0}
    ...
    voice_definitions = {
        ...
        "fr-fr": {
            "label": "French",
            "voices": [{"id": "ff_siwis", "name": "FF Siwis (Female)"}],
        },
        "es": {
            "label": "Spanish",
            "voices": [
...
async def run_setup(
    background_tasks: BackgroundTasks, model_type: Optional[str] = None
):
    """
    ...
    def setup_task():
        ...
        try:
            ...
            model_name = (
                "Standard (GPU)" if target_model == "gpu" else "Quantized (CPU)"
            )
...
async def switch_engine(background_tasks: BackgroundTasks, target_mode: str):
    ...
    if target_mode not in ["gpu", "cpu"]:
        raise HTTPException(
            status_code=400, detail="Invalid engine mode. Use 'gpu' or 'cpu'."
        )
...
async def download_specific_model(background_tasks: BackgroundTasks, model_type: str):
    ...
    if model_type not in ["gpu", "cpu"]:
        raise HTTPException(
            status_code=400, detail="Invalid model type. Use 'gpu' or 'cpu'."
        )
    ...
    model_path = models_dir / (
        "kokoro.onnx" if model_type == "gpu" else "kokoro.int8.onnx"
    )
...
def synthesize_with_pauses(
    ...
    print(f"\n{'='*60}")
    print(
        f"[PAUSE LOGIC v3.0 Parallel] Processing: '{text[:100]}{'...' if len(text) > 100 else ''}'"
    )
    ...
    if tts_tasks:
        print(
            f"[PARALLEL] Synthesizing {len(tts_tasks)} segments with {min(len(tts_tasks), 4)} threads..."
        )
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            # Submit all tasks
            future_to_idx = {
                executor.submit(
                    kokoro.create, t["text"], voice=voice, speed=speed, lang=lang
                ): t["index"]
                for t in tts_tasks
            }
    ...
    try:
        ...
        if not re.search(
            ...
            if has_pause_settings and has_punctuation:
                ...
                print(
                    f"  [MODE] Using standard synthesis (no punctuation or no pause settings)"
                )
        ...
        print(
            f"[CACHE SAVE] Audio saved to DB: {cache_key[:8]}... ({len(audio_bytes)} bytes)"
        )
...
async def open_file_location(filename: str):
    ...
    try:
        ...
        if system == "Windows":
            ...
            raise HTTPException(
                status_code=501, detail=f"Platform '{system}' not supported"
            )
