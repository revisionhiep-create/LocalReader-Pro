...
export async function selectDocument(item) {
    ...
    try {
        ...
        if ((item.currentPage || 0) === 0 && state.smartStartPage > 0) {
            ...
            showToast(`⚡ Skipped to start of content (Page ${state.smartStartPage + 1})`);
        } else {
...
export async function renderPage() {
    ...
    if (state.headerFooterMode !== 'off' && state.currentDoc) {
        try {
            const filterData = await fetchJSON(`/api/library/content/${state.currentDoc.id}/page/${state.currentPageIndex}`);
    ...
    state.sentences.forEach((s, i) => {
        ...
        span.className = `sentence ${i === state.currentSentenceIndex ? 'active-sentence' : ''}`;

        // Fix broken DIM tags caused by sentence splitting
        let cleanS = s;
        if (cleanS.includes('[DIM]') && !cleanS.includes('[/DIM]')) cleanS += '[/DIM]';
        if (!cleanS.includes('[DIM]') && cleanS.includes('[/DIM]')) cleanS = '[DIM]' + cleanS;

        if (cleanS.includes('[DIM]')) {
            const dimRegex = /\[DIM\](.*?)\[\/DIM\]/g;
            span.innerHTML = cleanS.replace(dimRegex, '<span class="dimmed-text">$1</span>');
        } else {
    ...
    if (currentSentencePreview) {
        currentSentencePreview.textContent = (currentSentence && typeof currentSentence === 'string')
            ? stripHTML(currentSentence) : "Ready";
    }
        ...
        if (lastItem) {
            ...
            if (verticalGap > (lastHeight * 0.4)) {
                ...
                if (!isTerminal && verticalGap < (lastHeight * 2.5)) {
                    ...
                    const isCJK = (c) => /[\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]/.test(c);

                    if (!text.endsWith(" ") && !str.startsWith(" ") && !isCJK(lastChar) && !isCJK(nextChar)) {
                        text += " ";
                    }
                } else {
                    text = text.trimEnd() + "\n";
                ...
                if (gap > Math.max(1.5, avgCharWidth * 0.25)) {
                    ...
                    if (!text.endsWith(" ") && !str.startsWith(" ") && !noSpaceBefore && !noSpaceAfter && (!isCJK(lastChar) || !isCJK(nextChar))) {
                        text += " ";
                    }
                }
            }
...
export async function processPdfBlob(blob, fileName) {
    ...
    reader.onload = async function () {
        try {
            ...
            await fetchJSON('/api/library/content', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: docId, pages: pagesText }) });
